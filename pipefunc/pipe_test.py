...
wavlistorder = ['WWL', 'Aileen', 'Robin', 'Mike']#
name_tag = 'Aileen'
# ipwav_list = sorted(p_main.joinpath('mono').glob('*.wav'))
ipwav_list = [p_main.joinpath('mono', f'{name_tag}_{x}.wav') for x in wavlistorder]
print(ipwav_list)

def importfiles(filepath):
    do_command(f'Import2: Filename={filepath}')

for ipwavpath in ipwav_list:
    print(str(ipwavpath))
    importfiles(ipwavpath)

#do_command(f'SelAllTracks:')
#do_command('NextTrack:')
#do_command(f'Select: Track=0')
'''