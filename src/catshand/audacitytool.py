...
from catshand.pipefunc import do_command, get_response, get_response_json, send_command
...
def importfiles(filepath, compressor=True):
    ...
    if compressor:
        do_command(
            f"Compressor: Threshold={str(-40)} NoiseFloor={str(-35)} Ratio={str(2)}"
        )
...
class audacitytool:
    def __init__(self, prj_path, ip_dir=None, hl_dir=None, premerge=None):
        ...
        if not self.configpath.is_file():
            raise Exception(
                "config file not found. Please run 'prjgen' command first to create the config file."
            )
        else:
        ...
        self.ENDCREDIT_PATH = material_root.joinpath(
            material_dict.get("endcredit_path")
        )
        self.TRANSITIONFLD = material_root.joinpath(
            material_dict.get("transition_path")
        )
    ...
    def importrecording(
        ...
        if single_track:
            track_names = [f"{prj_name}{ipformat}"]
        elif importall:
        ...
        for idx, track_name in enumerate(track_names):
            if track_name not in tracknamelist:
                ...
                importfiles(
                    self.IPFOLDER.joinpath(f"{track_name}"), compressor=compressor
                )
    ...
    def importhighlight(self):
        ...
        if hl_name not in tracknamelist:
            ...
            do_command("SelectAll:")
            do_command(
                f'SelectTracks: Mode="Set" Track="{len(self.namesall)+3}" TrackCount="1"'
            )
            do_command(f"SetTrack: Height={self.TRACK_HEIGHT}")
    ...
    def importmaterial(self):
        ...
        tracklengthlist = [
            trackinfo["end"] - trackinfo["start"] for trackinfo in trackinfo
        ]
        ...
        if "opmusic" not in tracknamelist:
            ...
            do_command(
                f"Compressor: Threshold={str(-12)} NoiseFloor={str(-35)} Ratio={str(2)}"
            )
            do_command(
                f'SelectTracks: Mode="Set" Track="{len(tracknamelist)}" TrackCount="1"'
            )
        ...
        if "endmusic" not in tracknamelist:
            ...
            do_command(
                f'SetClip: At="0" Start="{self.TRACK_OFFSET + self.TRACK_LENGTH_MAX + self.ENDMUSIC_OFFSET}"'
            )
        ...
        if "endcredit" not in tracknamelist:
            ...
            do_command(
                f'SetTrack: Name="endcredit" Height={int(self.TRACK_HEIGHT*0.6)}'
            )
            do_command(
                f'SetClip: At="0" Start="{self.TRACK_OFFSET + self.TRACK_LENGTH_MAX + self.ENDCREDIT_OFFSET + self.ENDMUSIC_OFFSET}"'
            )
    ...
    def _metadata2sectionlength(self, default_music="Middle_01.wav"):
        ...
        self.foldernamelist = [x.name for x in folderlist if x.name != ".DS_Store"]
        print(self.foldernamelist)
        fs = medatadict["op_fs"]
    ...
    def _importmusic(self):
        """AI is creating summary for _importmusic"""
        print("start import music")
        silence = 10
        music_offset = -3.5
        cumulative = 0
        ...
        for index, row in self.dfaddmusic.iterrows():
            ...
            if clipname not in tracknamelist:
                ...
                do_command(
                    f'SetTrack: Name="{clipname}" Height={int(self.TRACK_HEIGHT*0.6)}'
                )
                do_command(f'SelectTime: Start="{0}" End="{5}"')
                do_command(f'AdjustableFade: preset="SCurveIn"')
                do_command(
                    f'SetClip: At="0" Start="{self.TRACK_OFFSET + self.MIDDLE_TIMESTAMP + cumulative + music_offset}"'
                )
    ...
    def midedit(self):
        ...
        for idx in range(3):
            ...
            do_command(f'SelectTracks: Mode="Set" Track="{idx}" TrackCount="1"')
            do_command(
                f'SelectTime: Start="{self.TRACK_OFFSET + 50}" End="{self.TRACK_OFFSET + trackend}"'
            )
            do_command(f"SplitCut:")
            do_command(
                f'SelectTime: Start="{self.TRACK_OFFSET + 50 + 10}" End="{self.TRACK_OFFSET + 50 + 10}"'
            )
            do_command(f"Paste:")
        return
