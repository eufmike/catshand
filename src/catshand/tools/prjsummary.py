...
from catshand.openai_api import (
    convert_csv_to_txt,
    merge_tran_csv,
    openai_text,
    process_audio_file,
...
def prjsummary(args):
    ...
    openai_text(
        docdir.joinpath("merge.txt"),
        docdir.joinpath("summary.txt"),
        names=names,
        threads=threads,
    )
...
def add_subparser(subparsers):
    description = (
        "prjsummary creates the prejoct summary with transcript and time stamps"
    )
    ...
    optional_group.add_argument(
        ...
        "--input_dir",
        type=str,
        help="input folders with *.wav files. Default folder: 00_Raw_wav_processed_sil_removal",
    )
    optional_group.add_argument(
