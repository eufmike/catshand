...
def getvolume(volume_adj, track_cnt: int = 1):
    if not volume_adj is None:
        if len(volume_adj) == 1:
            volume_adj_arr = list(int(volume_adj[0])) * track_cnt
        elif len(volume_adj) > 1:
            volume_adj_arr = [int(x) for x in volume_adj]
            assert (
                len(volume_adj_arr) == track_cnt
            ), "Track count and volume adjustment count should be the same"
    else:
        volume_adj_arr = None
    print(volume_adj_arr)
    return volume_adj_arr
...
def trackmerger(args):
    ...
    volume_adj = args.volume_adjustment
    ...
    for parent_ipdir, parent_oppath in zip(parent_ipdirs, parent_oppaths):
        print(parent_ipdir)
        print(parent_oppath)
        child_ipfilelist = sorted(
            Path(parent_ipdir).glob(str(Path("**").joinpath(f"*.wav")))
        )
        ...
        print(len(child_ipfilelist))
        volume_adj_arr = getvolume(volume_adj, len(child_ipfilelist))
        track_all_idv = []
        for idx, ipfile in enumerate(child_ipfilelist):
            track_tmp = AudioSegment.from_wav(ipfile)
            if not volume_adj is None:
                track_tmp = track_tmp + volume_adj_arr[idx]
            if stereo:
                track_tmp = track_tmp.set_channels(2)
                if spatial:
                    print(left_adjust_volume[idx], right_adjust_volume[idx])
                    track_tmp = track_tmp.apply_gain_stereo(
                        left_adjust_volume[idx], right_adjust_volume[idx]
                    )
...
def add_subparser(subparsers):
    ...
    optional_group.add_argument(
        ...
        "--input_dir",
        type=str,
        help="input folders with audio files (.mp3, .wav, and .m4a).",
    )
    optional_group.add_argument(
        "-o", "--output_dir", type=str, help="output folders different from default"
    )
    optional_group.add_argument(
        "-s", "--stereo", action="store_true", help="export a stereo file"
    )
    optional_group.add_argument(
        "-sp", "--spatial", action="store_true", help="applied spatial effect"
    )
    optional_group.add_argument(
        "-v",
        "--volume_adjustment",
        type=str,
        nargs="+",
        help="applied volume percentage matrix",
    )
